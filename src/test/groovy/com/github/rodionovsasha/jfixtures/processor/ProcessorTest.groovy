...
class ProcessorTest extends Specification implements YamlVirtualDirectory {
    ...
    def "truncate cascade instructions when clean_method is 'truncate_cascade'"() {
        when:
        def instructions = load("basic_fixtures_with_truncate_cascade_clean_method.yml")

        then:
        instructions.size() == 2

        and:
        instructions.find { it instanceof InsertRow && it.table == "friends" }
        instructions.find { it instanceof CleanTable && it.cleanMethod == CleanMethod.TRUNCATE_CASCADE && it.table == "friends" }
    }
